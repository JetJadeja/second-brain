...
type SmartBatchBannerProps = {
  ...
  onConfirm: (cluster: BatchCluster) => void
  onDismiss: (cluster: BatchCluster) => void
}

export function SmartBatchBanner({ clusters, onConfirm, onDismiss }: SmartBatchBannerProps) {
  if (clusters.length === 0) return null

  return (
    <>
      {clusters.map((cluster) => (
        <div key={cluster.bucketId} className="flex items-center gap-3 rounded-md border-l-[3px] border-l-ember-500 bg-surface-100 px-4 py-3">
