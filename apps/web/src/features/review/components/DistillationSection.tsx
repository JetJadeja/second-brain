...
function getViewCountColor(viewCount: number): string {
  if (viewCount >= 10) return 'text-[var(--ember-500)]'
  if (viewCount >= 5) return 'text-[var(--surface-500)]'
  return 'text-[var(--surface-400)]'
}
...
export function DistillationSection({ nudges }: DistillationSectionProps) {
  ...
  if (nudges.length === 0) {
    return <p className="text-sm text-[var(--surface-400)]">Nothing needs distilling right now</p>
  }
      ...
      {nudges.map((nudge) => (
        <div key={nudge.id} className="flex items-center gap-3 rounded-lg px-3 py-2">
          <div className="flex-1 min-w-0">
            <p className="text-sm text-[var(--surface-600)] truncate">{nudge.title}</p>
