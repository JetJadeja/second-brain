...
export function OrphanSection({ orphans, actedIds, onFindConnections, onArchive }: OrphanSectionProps) {
  ...
  if (visible.length === 0) {
    return <p className="text-sm text-surface-400">No unconnected notes</p>
  }
      ...
      {visible.map((orphan) => (
        <div key={orphan.id} className="flex items-center gap-3 rounded-lg px-3 py-2 transition-opacity duration-200">
          <div className="flex-1 min-w-0">
            <p className="text-sm text-surface-600 truncate">{orphan.title}</p>
            <div className="flex items-center gap-2 mt-0.5">
              {orphan.bucket_name && (
                <span className="rounded bg-surface-200 px-1.5 py-0.5 text-[10px] font-medium text-surface-500">
                  {orphan.bucket_name}
                </span>
              )}
              <span className="text-[10px] text-surface-400">{formatRelativeTime(orphan.captured_at)}</span>
            </div>
          </div>
          <button
            type="button"
          ...
          <button
            ...
            className="shrink-0 rounded-md px-3 py-1 text-xs font-medium text-surface-300 hover:bg-surface-200 transition-colors"
          >
            Archive
          </button>
        </div>
