-- Add structured metadata columns for enriched search and display
ALTER TABLE notes ADD COLUMN topics TEXT[] DEFAULT '{}';
ALTER TABLE notes ADD COLUMN entities JSONB DEFAULT '[]';
ALTER TABLE notes ADD COLUMN content_quality JSONB DEFAULT NULL;

-- GIN index on topics for array containment queries
CREATE INDEX idx_notes_topics ON notes USING gin(topics);

-- Update the fts generated column to include topics
-- We need to drop and recreate because it's GENERATED ALWAYS
-- Note: This also drops the idx_notes_fts GIN index automatically
ALTER TABLE notes DROP COLUMN fts;
ALTER TABLE notes ADD COLUMN fts tsvector GENERATED ALWAYS AS (
  to_tsvector('english',
    coalesce(title, '') || ' ' ||
...
-- Recreate the GIN index on the updated fts column
CREATE INDEX idx_notes_fts ON notes USING gin(fts);

-- Recreate hybrid_search to use the updated fts column
-- The function references notes.fts by column name so it picks up the
-- new definition, but we re-declare to be safe after the DROP/ADD cycle.
